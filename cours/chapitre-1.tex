...
\begin{frame}
  ...
  \[
    y_t = \beta_1x_{t,1} + \beta_2x_{t,2} + \dots + \beta_Kx_{t,K} + \varepsilon_t
  \]
  ...
  \item La nature nous donne un échantillon $\{y_t,x_{t,1},\ldots,x_{t,K}\}_{t=1}^T$ ($T$ est le nombre d'observations).\newline
...
\begin{frame}
  ...
  \begin{figure}
    ...
    \begin{subfigure}{0.48\textwidth}
      \scalebox{.3}{
    \input{../pgf/chapitre-1/sample-nonstochastic-x.tex}}
  ...
  \begin{subfigure}{0.48\textwidth}
    \scalebox{.3}{
    \input{../pgf/chapitre-1/sample-stochastic-x.tex}}
...
\begin{frame}
  ...
  \item Ce modèle peut être représenté matriciellement sous la forme :
    ...
    \item On notera $\mathbf x_t = (x_{t,1},x_{t,2}, \dots, x_{t,K})$ la t-ième observation pour les exogènes (un vecteur $1\times K$).
...
\begin{frame}
  ...
  \begin{exemple}{Équation de salaire}
    ...
    \[
      \underbrace{\log w_i}_{y_i} = \beta_1 + \beta_2 \underbrace{educ_i}_{x_{i,2}}
      + \beta_3 \underbrace{exper_i}_{x_{i,3}} + \beta_4 \underbrace{exper_i^2}_{x_{i,4}}
      + \varepsilon_i
    \]
...
\begin{frame}[c]
  ...
  \begin{center}
    \scalebox{.86}{\input{../tikz/chapitre-1/objectif-mco.tex}}
  \end{center}
...
\begin{frame}[c]
  ...
  \begin{center}
    \scalebox{.82}{\input{../tikz/chapitre-1/projection-orthogonale.tex}}
  \end{center}
...
\begin{frame}
  ...
  \begin{defn}{}
    On note $\hat y_t = \mathbf x_t \hat{\mathbf b}$ le prédicteur de $y_t$, $\bar y = T^{-1}\sum_{t=1}^T y_t$ la moyenne arithmétique, et on définit~:
    \[
      \begin{split}
...
\begin{frame}[c]
  ...
  \begin{figure}
    \centering
    \scalebox{.44}{
      \input{../pgf/chapitre-1/anscombe.tex}}
...
\begin{frame}[c]
  ...
  \begin{figure}
    \centering
    \scalebox{.46}{\input{../pgf/chapitre-1/r2-trompeur.tex}}
...
\begin{frame}
  ...
  \begin{figure}
    ...
    \begin{subfigure}{0.48\textwidth}
      \scalebox{.3}{
    \input{../pgf/chapitre-1/slope-estimate-sample-nonstochastic-x.tex}}
    \caption{$x$ déterministe.}
    \label{fig:slope:a}
  \end{subfigure}
  \hfill
  \begin{subfigure}{0.48\textwidth}
    \scalebox{.3}{
    \input{../pgf/chapitre-1/slope-estimate-sample-stochastic-x.tex}}
    \caption{$x$ stochastique.}
    \label{fig:slope:b}
  \end{subfigure}
...
\begin{frame}[c]
  ...
  \begin{figure}
    \centering
    \scalebox{.46}{\input{../pgf/chapitre-1/signal-bruit.tex}}
...
\begin{frame}[c]
  ...
  \begin{figure}
    \centering
    \scalebox{.46}{\input{../pgf/chapitre-1/gauss-markov.tex}}
...
\begin{frame}
  ...
  \begin{figure}
    \centering
      \scalebox{.40}{
    \input{../pgf/chapitre-1/ols-convergence.tex}}
...
\begin{frame}[c]
  ...
  \begin{center}
    \scalebox{.80}{\input{../tikz/chapitre-1/mse-s2-mv.tex}}
  \end{center}
...
\begin{frame}
  ...
  \begin{exemple}{Rendements d'échelle}
    ...
    \[
      \underbrace{\log Q_t}_{y_t} = \beta_1 + \beta_2 \underbrace{\log K_t}_{x_{t,2}}
      + \beta_3 \underbrace{\log L_t}_{x_{t,3}} + \varepsilon_t
    \]
...
\begin{frame}[c]
  ...
  \begin{center}
    \scalebox{.78}{\input{../tikz/chapitre-1/region-rejet-student.tex}}
  \end{center}
...
\begin{frame}[c]
  ...
  \begin{center}
    \scalebox{.74}{\input{../tikz/chapitre-1/ellipse-vs-rectangle.tex}}
  \end{center}
...
\begin{frame}[c]
  ...
  \begin{figure}
    \centering
    \scalebox{.46}{\input{../pgf/chapitre-1/mco-contraints.tex}}
...
\begin{frame}[c]
  ...
  \begin{center}
    ...
    \input{../pgf/chapitre-1/sortie-mincer.tex}

    \bigskip

    \footnotesize
    \input{../pgf/chapitre-1/sortie-mincer-synthese.tex}
  \end{center}
...
\begin{notes}
  ...
  \item Les données et le code sont dans le dépôt~:
    \texttt{data/\allowbreak chapitre-1/\allowbreak wage1.csv} et
    \texttt{routines/\allowbreak chapitre-1/\allowbreak mincer.py}. Il s'agit de l'échantillon \texttt{wage1}
...
\begin{frame}
  ...
  \begin{defn}{}
    Soit $\{X_n\}$ une suite de variables aléatoires de fonctions de
    répartition $\{F_n\}$. On dit que $X_n$ converge en loi vers $X$, de
    fonction de répartition $F$, si~:
    \[
      \underset{n\rightarrow\infty}{\lim} F_n(x) = F(x)
    \]
    en tout point $x$ où $F$ est continue. On note~:
    \[
      X_n \xlongrightarrow[n\rightarrow\infty]{\text{loi}} X
    \]
  \end{defn}

  \bigskip

  \begin{theorem}[Slutsky]\label{thm:Slutsky:mixte}
    Si $X_n \xlongrightarrow[n\rightarrow\infty]{\text{loi}} X$ et
    $Y_n \xlongrightarrow[n\rightarrow\infty]{\text{proba}} c$, où $c$ est une
    constante, alors~:
    \[
      Y_nX_n \xlongrightarrow[n\rightarrow\infty]{\text{loi}} cX
      \quad\text{et}\quad
      X_n+Y_n \xlongrightarrow[n\rightarrow\infty]{\text{loi}} X+c
    \]
  \end{theorem}
...
\begin{notes}
  ...
  \item La convergence en probabilité implique la convergence en loi, la
    réciproque est fausse (sauf si la limite est une constante). C'est la
    convergence en loi qui nous intéresse pour construire des tests~: elle
    fournit une loi approchée pour la statistique, alors que la convergence en
    probabilité ne donne que le point vers lequel elle se tasse.\newline

  \item Le théorème~\ref{thm:Slutsky:mixte} est l'outil qui permet de remplacer,
    dans une statistique, une matrice de moments empiriques par sa limite en
    probabilité sans changer la loi asymptotique. Nous l'utiliserons
    systématiquement~: c'est lui qui autorise à écrire
    $\sqrt{T}\left(\hat{\mathbf b}-\beta\right) = \left(\nicefrac{X'X}{T}\right)^{-1}\nicefrac{X'\varepsilon}{\sqrt T}$
    puis à traiter $\left(\nicefrac{X'X}{T}\right)^{-1}$ comme s'il valait
    $Q^{-1}$.\newline

  \item \textbf{Attention}, contrairement au théorème~\ref{thm:Slutsky}, la
    limite de $Y_n$ doit ici être une constante. Le produit de deux suites qui
    convergent en loi ne converge pas en loi vers le produit des limites.
...
\begin{frame}
  ...
  \begin{theorem}[Lindeberg-Lévy]\label{thm:tcl}
    Soit $\{X_n\}$ une suite de variables aléatoires indépendantes et
    identiquement distribuées, d'espérance $\mu$ et de variance $\sigma^2$
    finie. Alors~:
    \[
      \sqrt{n}\left( \bar X_n - \mu \right) \xlongrightarrow[n\rightarrow\infty]{\text{loi}} \mathcal N\left( 0,\sigma^2 \right)
    \]
    où $\bar X_n = \frac{1}{n}\sum_{i=1}^n X_i$.
  \end{theorem}
  ...
  \item Version multivariée~: si les $\bm X_i$ sont iid dans $\mathbb R^p$
    d'espérance $\bm\mu$ et de variance $\Sigma$ finie, alors
    $\sqrt{n}\left( \bar{\bm X}_n - \bm\mu \right) \xlongrightarrow[n\rightarrow\infty]{\text{loi}} \mathcal N\left( 0,\Sigma \right)$.\newline

  \item[\dbend] L'existence du moment d'ordre deux n'est pas une formalité~: il
    existe des variables aléatoires parfaitement raisonnables qui n'en ont pas.
...
\begin{notes}
  ...
  \item La loi des grands nombres nous dit vers quoi $\bar X_n$ converge, le
    théorème~\ref{thm:tcl} nous dit à quelle vitesse et sous quelle forme~: en
    $\nicefrac{1}{\sqrt n}$, et avec une loi normale, \emph{quelle que soit} la
    loi des $X_i$. C'est ce qui rend l'inférence asymptotique possible sans rien
    supposer sur la loi des erreurs.\newline

  \item Les hypothèses peuvent être considérablement affaiblies (les $X_i$
    peuvent être hétéroscédastiques, ou dépendants, pourvu que la dépendance
    s'estompe assez vite). Mais elles ne peuvent pas être supprimées. Si la
    variance n'existe pas, il n'y a pas de théorème central limite~: la moyenne
    empirique de variables de Cauchy iid suit une loi de Cauchy, quelle que soit
    la taille de l'échantillon, et ne converge vers rien.\newline

  \item Nous rencontrerons cette pathologie au \chapref{3}{III}, à propos des
...
\begin{frame}
  ...
  \begin{center}
    \input{../tikz/chapitre-1/chi-squared.tex}
  \end{center}
...
\begin{frame}
  ...
  \begin{center}
    \input{../tikz/chapitre-1/student.tex}
  \end{center}
...
\begin{frame}
  ...
  \begin{center}
    \input{../tikz/chapitre-1/fisher-snedecor-1.tex}
  \end{center}
...
\begin{frame}
  ...
  \begin{center}
    \input{../tikz/chapitre-1/fisher-snedecor-2.tex}
  \end{center}
...
\begin{frame}
  ...
  \begin{center}
    \input{../tikz/chapitre-1/fisher-snedecor-3.tex}
  \end{center}
